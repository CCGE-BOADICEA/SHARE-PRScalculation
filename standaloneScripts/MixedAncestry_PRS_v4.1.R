...
propsFile  <- "Examples/MixedProportions/example_valuesA.csv"    #Raw PGS and the ancestry proportions
parsFile   <- "PRSmodels_CanRisk/BCAC_307_PRS-UKB_all.prs"       #SNPs frequencies and logOR, and PGS model parameters
...
if (any(monoFlag)){
  ...
  stDev <- stDev + 4*sum((SNPtmp1 - (SNPtmp2**2)) * (SNPtab$log_OR**2))
  stDev <- sqrt(stDev)
