...
if any(sampleProp<0):
    raise ValueError("Error in the input file: negative proportions!")
if not( 0.99< np.sum(sampleProp) <1.01 ):
    raise ValueError("Error in the input file: proportions do not add up to 1!")
sampleProp = sampleProp/np.sum(sampleProp)

# SNPs log(OR) and EAFs
with open(File_LociPars, "r") as data:
    while True:
        line = data.readline()
        if not line.startswith('#'): break
    tmpTab = np.genfromtxt(data, delimiter=',',skip_header=4,usecols = (4,6,7,8,9),comments='#')
logORs = tmpTab[:,0]
EAFtab = tmpTab[:,1:]
